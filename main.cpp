...
DLLEXPORT void extractShapelet(int N, int C,char *filePath, int maxLength, int minLength) {
	int i, j;
	double t1, t2;
	t1 = clock();

	//const char* filePath = "C://FinalProject//Test//shapelet//Car_TRAIN";
	...
	start = 10;
	endLength = maxLen;
	if(minLen !=NULL) start = minLength;
	if(maxLen !=NULL) endLength = maxLength;
	stepSize = 10;
