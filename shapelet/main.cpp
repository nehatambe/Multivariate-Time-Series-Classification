...
DLLEXPORT char* get_error_string() {
	return error_string;
}
...
DLLEXPORT int recursiveShapelet(timeSeries ** T, int N, int C, int nodeId)
{
	...
	for (k = 0; k < N; k++)
	{
		...
		for (len = start; len <= endLength; len += stepSize)
		{
			refCount = 0;
...
DLLEXPORT void extractShapelet(int N, int C,char *filePath, char *writeFilePath, int maxLength, int minLength, int stepSizeNum) {
	int i, j;
	double t1, t2;
	t1 = clock();
	...
	start = 10;
	endLength = maxLen;
	if(minLength !=NULL) start = minLength;
	if(maxLength !=NULL) endLength = maxLength;
